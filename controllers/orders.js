...
export const getIncompleteOrders = async (req, res) => {
  ...
  try {
    let query = 'SELECT * FROM orders WHERE user_id = ? AND status NOT IN ("訂單完成", "顧客取消訂單", "商家取消訂單")'
    let countQuery = 'SELECT COUNT(*) AS count FROM orders WHERE user_id = ? AND status NOT IN ("訂單完成", "顧客取消訂單", "商家取消訂單")'
    const queryParams = [req.user.id]
    const countParams = [req.user.id]

    // Filter conditions (status, deliveryDate, userName, orderNumber)
    if (status) {
      query += ' AND status = ?'
      countQuery += ' AND status = ?'
      queryParams.push(status)
...
export const getCompletedOrders = async (req, res) => {
  ...
  try {
    let query = 'SELECT * FROM orders WHERE user_id = ? AND status IN ("訂單完成", "顧客取消訂單", "商家取消訂單")'
    let countQuery = 'SELECT COUNT(*) AS count FROM orders WHERE user_id = ? AND status IN ("訂單完成", "顧客取消訂單", "商家取消訂單")'
    const queryParams = [req.user.id]
    const countParams = [req.user.id]
...
export const getAllOrders = async (req, res) => {
  const pool = req.pool
  const { page = 1, limit = 5, status, deliveryDate, startDate, endDate, userName, orderNumber } = req.query
  const offset = (page - 1) * limit
  ...
  try {
    ...
    if (role === 2) {
      ...
      `
      totalRevenueQuery = `
        SELECT SUM(p.price * op.quantity) AS totalRevenue
        FROM orders o
    ...
    if (status) {
      ...
      totalRevenueQuery += ' AND o.status = ?'
      queryParams.push(status)
      countParams.push(status)
    ...
    if (deliveryDate) {
      ...
      totalRevenueQuery += ' AND o.delivery_date = ?'
      queryParams.push(deliveryDate)
      countParams.push(deliveryDate)
      revenueParams.push(deliveryDate)
    }

    if (startDate && endDate) {
      const endDateWithTime = `${endDate} 23:59:59` // 確保包含結束日期的整天
      query += ' AND o.delivery_date BETWEEN ? AND ?'
      countQuery += ' AND o.delivery_date BETWEEN ? AND ?'
      totalRevenueQuery += ' AND o.delivery_date BETWEEN ? AND ?'
      queryParams.push(startDate, endDateWithTime)
      countParams.push(startDate, endDateWithTime)
      revenueParams.push(startDate, endDateWithTime)
    }

    if (userName) {
      query += ' AND u.account = ?'
      countQuery += ' AND u.account = ?'
      totalRevenueQuery += ' AND u.account = ?'
      queryParams.push(userName)
      countParams.push(userName)
      revenueParams.push(userName)
    }

    if (orderNumber) {
      query += ' AND o.oid LIKE ?'
      countQuery += ' AND o.oid LIKE ?'
      totalRevenueQuery += ' AND o.oid LIKE ?'
      queryParams.push(`%${orderNumber}%`)
      countParams.push(`%${orderNumber}%`)
      revenueParams.push(`%${orderNumber}%`)
    }
    ...
    const [orders] = await pool.query(query, queryParams)
    const [countResult] = await pool.query(countQuery, countParams)
    const [revenueResult] = await pool.query(totalRevenueQuery, revenueParams)

    if (orders.length === 0) {
      return res.status(200).json({ success: true, message: '目前沒有訂單', result: [], totalPages: 0, totalRevenue: 0 })
    }
    ...
    res.status(200).json({ success: true, message: '', result: ordersWithProducts, totalPages, totalRevenue })
  } catch (error) {
...
// 新增一個API來獲取所有符合條件的訂單數據
export const getAllCompletedOrders = async (req, res) => {
  const pool = req.pool
  const { startDate, endDate, userName, orderNumber } = req.query
  const uid = req.query.uid
  const role = req.user.role
  ...
  try {
    ...
    if (orders.length === 0) {
      return res.status(200).json({ success: true, message: '目前沒有符合條件的訂單', result: [] })
    }
    ...
    res.status(200).json({ success: true, message: '', result: ordersWithProducts })
  } catch (error) {
    console.error('Get completed orders error:', error)
    res.status(500).json({ success: false, message: '未知錯誤' })
  }
